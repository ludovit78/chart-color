...
function handleFiles(files){const file=files&&files[0];if(!file)return;if(file.name.toLowerCase().endsWith(".pdf")){loadPdf(file);return;}const r=new FileReader();r.onload=()=>loadChartText(String(r.result||""));r.readAsText(file);}
function loadChartText(text){state.raw=String(text||"");const box=$("#src");if(box)box.value=state.raw;parseCurrent();}
